...
namespace AnotherExternalMemoryLibrary
{
    ...
    {
        private static void WriteProcessMemory_(ptr hProcess, ptr lpBaseAddress, byte[] bytes)
        {
            ...
            Win32.WriteProcessMemory(hProcess, lpBaseAddress, bytes, bytes.Length, out ptr _);
            Win32.VirtualProtectEx(hProcess, lpBaseAddress, bytes.Length, oldProtection, out Win32.MemoryProtection _);
        }
        public static void Write<T>(ptr pHandle, ptr addr, T value) where T : struct
        {
            byte[] data = value.ToByteArray();
            WriteProcessMemory_(pHandle, addr, data);
        }
        public static void Write<T>(ptr pHandle, ptr addr, params T[] array) where T : struct
        {
